...
# @File    : generative_models_lightning/diffusion/resample.py
# @IDE     : vscode
...
class LossAwareSampler(ScheduleSampler):
    def update_with_local_losses(
        self, local_ts: torch.Tensor, local_losses: torch.Tensor
    ) -> None:
        """
        ...
        max_bs = np.max(batch_sizes)

        timestep_batches = [torch.zeros(size=max_bs).to(local_ts) for _ in batch_sizes]
        loss_batches = [torch.zeros(size=max_bs).to(local_losses) for _ in batch_sizes]
        dist.all_gather(timestep_batches, local_ts)
        dist.all_gather(loss_batches, local_losses)
        timesteps = [
            int(x.item()) for y, bs in zip(timestep_batches, batch_sizes) for x in y[:bs]
        ]
        losses = [float(x.item()) for y, bs in zip(loss_batches, batch_sizes) for x in y[:bs]]
        self.update_with_all_losses(timesteps, losses)

    @abstractmethod
    def update_with_all_losses(self, ts: Sequence[int], losses: Sequence[float]) -> None:
        """
        Update the reweighting using losses from a model.
