...
def adaptive_l2_loss(
    ...
    r"""Compute MeanFlow's adaptive per-example L2 objective.

    The adaptive weight is detached, so gradients only flow through the
    ordinary squared error. ``valid_mask`` can be used to ignore padded or
    otherwise invalid spatial values.
    """
...
class MeanFlowLoss(_Loss):
    r"""MeanFlow objective using a directional derivative of the model.

    ``path_sample`` supplies the interpolated state ``x_t``, instantaneous
    velocity ``dx_t``, and upper time ``t``. The lower time ``r`` remains a
    separate input because the shared :class:`PathSample` represents a
    single-time probability path.

    Args:
        num_classes: Number of classes for the legacy class-conditioned
            MeanFlow model. Its unconditional label is ``num_classes``. If
            the condition is dense, an all-zero tensor is used instead.
        cfg_ratio: Probability of dropping an entire condition example.
        cfg_scale: Classifier-free guidance distillation scale. ``None``
            disables CFG distillation.
        cfg_uncond: With ``"v"``, dropped examples retain the exact path
            velocity as their target, matching the reference implementation.
        jvp_api: ``"autograd"`` or the legacy ``"funtorch"`` spelling.
            ``"functorch"`` and ``"torch.func"`` are also accepted.
        gamma: Exponent used by the adaptive L2 objective.
        c: Positive stability constant used by the adaptive L2 objective.
    """
    ...
    def _unconditional_condition(self, condition: Tensor) -> Tensor:
        # The reference class-conditioned model uses one extra embedding at
        # index ``num_classes``. Dense conditions instead use a zero image/map.
    ...
    def forward(
        ...
        """Return the adaptive MeanFlow loss and detached MSE metric."""
        x_t = path_sample.x_t
        ...
        if condition is not None:
            ...
            condition_mask = expand_tensor_like(
                input_tensor=cfg_mask,
                expand_to=condition,
            )
            condition_for_model = torch.where(
                condition_mask,
                unconditional,
                condition,
            )
